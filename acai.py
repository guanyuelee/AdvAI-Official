    ...
    def model(self, latent, depth, scales, advweight, advdepth, reg):
        ...
        def disc(x):
            return tf.reduce_mean(
                layers.encoder(x, scales, depth, latent, 'disc'),
                axis=[1, 2, 3])
        ...
        gap_ops = classifiers.gap_classifier(tf.stop_gradient(encode), l, self.nclass)
        gap_loss = tf.reduce_mean(gap_ops.loss)
        gap_vars = tf.global_variables('gap_classifier')
        gap_weight = [v for v in gap_vars if v.name == "gap_classifier/logits/kernel:0"]
        ...
        with tf.control_dependencies(update_ops):
            ...
            train_gap = tf.train.AdamOptimizer(FLAGS.lr).minimize(
                gap_loss, var_list=gap_vars)
        ops = train.AEOps(x, h, l, encode, decode, ae,
                          tf.group(train_ae, train_d, train_xl, train_gap),
                          train_xl,
                          classify_latent=xops.output,
                          gap_classify = gap_ops.output,
